...
//I think this class may no longer be necessary but I don't want to get rid of it just in case
...
public class ShearsSnipInteractions {
    ...
    private static int getFortuneLevel(ItemStack tool, Level level) {
        return tool.getEnchantmentLevel(level.holderOrThrow(Enchantments.FORTUNE));
    ...
    public static void onShearCrop(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (state.getBlock() instanceof HangingLilacLeavesBlock) {
            ...
            if (age >= HangingLilacLeavesBlock.MAX_AGE) {
                ...
                int extraDrops = rollFortuneExtras(rand, getFortuneLevel(tool, level));
        ...
        if (state.getBlock() instanceof MushroomColonyBlock mushroomColony) {
            ...
            if (age > 0) {
                ...
                ItemStack drop = new ItemStack(mushroomColony.mushroomType.value(), 1 + extraDrops);
                dropItemInFront(level, player, drop);
    ...
    public static void onShearChicken(PlayerInteractEvent.EntityInteract event) {
        ...
        int extraFeathers = rollFortuneExtras(rand, getFortuneLevel(tool, event.getLevel()));
    ...
    public static void onShearSheep(PlayerInteractEvent.EntityInteract event) {
        ...
        int extraWool = rollFortuneExtras(rand, getFortuneLevel(tool, level));
